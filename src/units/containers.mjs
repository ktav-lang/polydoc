...
//
// Because the source is Markdown, nothing needs escaping — content's own
// code fences are written as themselves. The cost is that a line
// beginning ">>>>> lang=" can no longer appear inside a block, which is
// why a duplicate or a stray one is rejected here rather than silently
// taken as a boundary.
//
...
// THIS LIBRARY'S REQUIREMENT ON TOP OF THE FORMAT: the set is exactly the
// configured LANGS, in every file. Comparing each file against one declared
// set is what makes "the same languages everywhere" true — not a
...
function validateBodySourceShape(unit, k, src, label = bodyFileName(k)) {
  ...
  if (unexpected.length > 0) {
    reject(`${label}: unexpected language block(s) ${unexpected.join(', ')} ` +
      `(this document is written in ${LANGS.join(', ')})`);
  }
