...
function rollbackTransaction(specDir, contentDir, state, writeSync, guard, unlinkSync = fs.unlinkSync) {
  ...
  // Re-checking every exact derived name makes rollback idempotent after a
  ...
  for (let index = outputs.length - 1; index >= 0; index--) {
    try {
      ...
      errors.push(`${outputs[index].root}/${outputs[index].name}: could not restore backup ${backup}: ${e.message}`);
    }
  }
  return errors;
}
...
function recoverCommittedTransaction(specDir, contentDir, state, writeSync, guard, unlinkSync = fs.unlinkSync) {
  ...
  const outputs = transactionOutputs();
  for (let index = state.cleanupIndex; index < outputs.length; index++) {
