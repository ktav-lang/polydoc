...
import { LANGS, OUT_FILES, README_FILES } from '../config.mjs';
import {
  lockCandidatePath,
  lockClaimEqual,
  lockIdentityEqual,
...
// NOTE: kept byte-identical to the pre-extraction ktav names on purpose.
// A consumer's existing on-disk artifacts, crash-injection env vars and
// error-message assertions (spec's own test suite has ~15 files pinned to
// these exact strings) must keep working unmodified after this engine
// moved into its own package. Rebranding these is a cosmetic follow-up for
// whenever this library is actually prepared for outside consumption, not
...
// Recomputed on every call, never cached at module-evaluation time: LANGS/
// OUT_FILES/README_FILES are only known once the caller's configure() has
// run, which happens strictly after this module is first imported.
...
function transactionOutputNames() {
  return new Set(transactionOutputs().map(({ name }) => name));
}
...
function transactionArtifactName(name) {
  ...
  return [...transactionOutputNames()].some((output) => {
...
function validateJournalState(state, specDir, contentDir) {
  ...
  if (!exactObjectKeys(state, TRANSACTION_STATE_KEYS) ||
      ...
      !Array.isArray(state.outputs) || state.outputs.length !== outputs.length) {
    fail('transaction journal has an invalid schema; ambiguous data was left untouched');
  }
  const existingCount = outputs.filter(({ root, name }) => {
  ...
  if (!inRange(state.backupIndex, existingCount) || !inRange(state.installIndex, outputs.length) ||
      !inRange(state.cleanupIndex, outputs.length) || !inRange(state.rollbackIndex, outputs.length)) {
  ...
  if ((state.phase === 'committed' || state.phase === 'cleaning') &&
      (state.backupIndex !== existingCount || state.installIndex !== outputs.length || state.rollbackIndex !== 0)) {
    fail('transaction journal has invalid commit ordering; ambiguous data was left untouched');
  }
  if (state.phase === 'rollback' && state.rollbackIndex > outputs.length) {
...
function outputPaths(specDir, contentDir, state, index) {
  const { root, name } = transactionOutputs()[index];
  const directory = root === 'spec' ? specDir : contentDir;
  return {
...
function transactionOutputBytes(bufs, readmeBufs) {
  return transactionOutputs().map(({ root, name }) => Buffer.from(
    root === 'spec'
