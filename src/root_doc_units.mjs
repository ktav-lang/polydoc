...
// units instead of one big `<NAME>.source.md` file, mirroring content's
// meta.js + body-N.md + manifest.js shape. Used by root_docs.mjs for the
// configured repository-root documents, and reusable by a consumer's own
// per-version README as the alternative to a single-file source.
//
// DELIBERATELY NARROWER than validateContentDir:
...
// - No nested group directories. Root documents are typically far smaller
//   than a full spec corpus, so a flat manifest of unit names is enough.
// - No section-inventory lock. That lock exists to catch a spec's own
//   claimed-vs-actual unit count drift; root documents have no equivalent
...
// (BODY_LINE_LIMIT/BODY_TARGET_LINES via bodySplitPlan).
//
// No journalled transaction (see root_docs.mjs's own header comment for
// why root documents stay on the plain fs.writeFileSync path) — this
...
/// to fall back to a single-file source path instead.
...
/// parts (array of `{[lang]: string, ...}`, one per body-N.md), already
/// validated against the same split-size mandate every content unit obeys.
...
/// `<repoRoot>/root-docs/README`. Returns `{[lang]: string, ...}`
/// concatenated body text, in manifest order — the same shape a single-file
/// source path returns, so root_docs.mjs's caller does not need to know
