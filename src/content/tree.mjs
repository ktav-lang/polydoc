...
export function readJsonDefault(filePath) {
  let buf;
  try {
...
export function structuralMeta(unit, meta) {
  return {
    unit,
    kind: meta.kind,
    number: meta.number,
...
export function substituteReleaseTokens(text, release) {
  return text.replaceAll(VERSION_TOKEN, release.version)
    ...
    .replaceAll(MINOR_LINE_TOKEN, minorLineOf(release.version));
}

// Facts about this corpus and this builder that author instructions would
// otherwise restate by hand, in every language.
...
// Tokens that are MEANT to reach the reader spelled as tokens. Author
// instructions document the release-token mechanism, so they have to be
// able to write @@VERSION@@ in prose; substituting there would replace
// every explanation of the mechanism with the version number.
...
export function readRelease(contentDir) {
...
export function validateSectionInventoryLock(manifest, lockPath, units = null, expectedVersion) {
  ...
  if (lock.format !== SECTION_INVENTORY_LOCK_FORMAT) {
...
export async function writeSectionInventoryLock(contentDir, lockPath, release) {
  ...
  const next = {
    format: SECTION_INVENTORY_LOCK_FORMAT,
  ...
  if (fs.existsSync(lockPath)) {
    try {
      ...
      process.stdout.write(
        `polydoc: existing lock could not be decoded; writing a fresh one\n`);
    }
  }
  ...
  if (lines.length === 0) {
    process.stdout.write('polydoc: section inventory lock is already current; nothing written\n');
    return;
  }

  process.stdout.write(
    'polydoc: section inventory lock changes:\n' + lines.join('\n') + '\n');
...
// Return the exact cut offsets prescribed for one language body. Offsets
// are JavaScript string offsets, matching the offsets used when the
// decoded body parts are concatenated below.
...
export function bodySplitPlan(parts) {
  ...
  // language — a file you can open and compare, rather than N slices
...
export function validateBodySplitting(unit, meta, parts) {
...
// Shared by content units and root-document units — the contract does not
// depend on what generates the heading (or on there being one at all).
...
export async function validateContentDir(contentDir, options = {}) {
  ...
  const lockPath = options.requireSectionInventoryLock || options.sectionInventoryLockPath
    ? options.sectionInventoryLockPath
    : null;
  if (options.requireSectionInventoryLock && lockPath === null) {
    fail('validateContentDir: requireSectionInventoryLock requires sectionInventoryLockPath');
  }
  ...
  if (!actualFiles.has(README_SOURCE_FILE) && !hasUnits) {
    fail(`required file "${README_SOURCE_FILE}" (or "${README_UNITS_DIR}/manifest.js") is missing under content/ (the READMEs must come from one source)`);
  }
  if (actualFiles.has(README_SOURCE_FILE) && hasUnits) {
  ...
  for (let idx = 0; idx < manifest.length; idx++) {
    ...
    for (const name of present) {
      ...
      if (/\.md$/.test(name) && LANGS.includes(name.slice(0, -3))) {
...
export async function buildBuffers(contentDir, options = {}) {
  requireConfigured();
  if (OUT_FILES === null || README_FILES === null) {
    fail('buildBuffers: configure({ outFileNames, readmeFileNames }) must be set before use');
  }
  const { manifest, units, readmes, release } = await validateContentDir(contentDir, options);

  const outputs = Object.fromEntries(LANGS.map((l) => [l, []]));
  const pieces = Object.fromEntries(LANGS.map((l) => [l, []]));
  const lens = Object.fromEntries(LANGS.map((l) => [l, 0]));
  ...
  // Author instructions carry facts this builder owns — how many units
  // exist, what the split constants are. Substituting them here is what
  // stops the document drifting from the corpus it describes. Derived
  // facts are substituted; RELEASE tokens deliberately are not: a document
  // explaining the token mechanism has to be able to write @@VERSION@@ in
  // prose and have it survive to the reader.
  const readmeBufs = {};
  for (const lang of LANGS) {
  ...
  for (const lang of LANGS) {
    for (const token of [VERSION_TOKEN, DATE_TOKEN, MINOR_LINE_TOKEN]) {
...
export function assertRegularDestination(destination) {
  let stat;
  try {
